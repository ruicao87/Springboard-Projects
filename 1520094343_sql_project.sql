...
SELECT COUNT(membercost) AS no_charge
FROM  Facilities 
WHERE membercost = 0
...
SELECT name, 
       ...
       ELSE  'cheap' END AS maintenance_fee
    FROM  Facilities 
...
SELECT DISTINCT f.name AS facilities_name,
       CONCAT(m.firstname,' ',m.surname) AS members_name
     FROM Bookings b
...
SELECT d.facilities_name,
       ...
       d.cost
   FROM (
     SELECT b.memid, f.name AS facilities_name, 
     CASE WHEN b.memid =0 THEN b.slots * f.guestcost
     ELSE b.slots * f.membercost  END AS cost
     FROM  Bookings  b
     JOIN Facilities  f ON b.facid = f.facid
  ...
  JOIN Members m ON d.memid = m.memid
  WHERE d.cost >30
  ORDER BY cost DESC
